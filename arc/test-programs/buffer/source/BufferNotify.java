...
public class BufferNotify {
    ...
    static private int _averageP = 4; // DK: was 16
    /** number of consumers in an average concurrency run  **/
    static private int _averageC = 2; // DK: was 8
    ...
    static public class PCThreads{
        ...
        void startPCT(){
            ...
            while (i<numOfProducers || j<numOfConsumers){
                if (i<numOfProducers) {
                    //System.out.println("Starting producer "+i); // DK
                    threadProducer[i].start();
                }
                if (j<numOfConsumers) {
                    threadConsumer[j].start();
                    //System.out.println("Starting consumer "+i); // DK
                }
                i++;
                if (i<numOfProducers) {
                    threadProducer[i].start();
